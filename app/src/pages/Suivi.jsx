    ...
    return (
      <div>
        ...
        <Champ etiquette={t('suivi.code')} aide={t('suivi.saisirCodeAide')}>
          <input
            className="champ text-center text-xl font-black tracking-[.15em] uppercase nombres-latins"
            dir="ltr" maxLength={14} autoFocus inputMode="text" autoCapitalize="characters"
            placeholder="66112233AB"
            value={saisie}
            onChange={(e) => setSaisie(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))}
          />
        </Champ>
        <Bouton taille="grand" className="w-full" disabled={saisie.length < 5}
    ...
    return (
      <div>
        ...
        <div className="mt-4 flex justify-center">
          <Lien vers="/"><Bouton variante="secondaire">{t('commun.accueil')}</Bouton></Lien>
        </div>
      </div>
    )
  }
